...
much'', and that is what precision measures. Note that because the
rules-only catalogue emits a candidate threat for every
(component, STRIDE) pair, there are no true negatives by
construction; accuracy therefore degenerates to precision (both
0.8409) and is reported in the table only for completeness ---
precision and $F_1$ are the informative metrics here. The 21 false positives
